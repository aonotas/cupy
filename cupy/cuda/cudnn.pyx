...
cdef extern from "cupy_cudnn.h" nogil:
    ...
    int cudnnDestroyPersistentRNNPlan(RNNDescriptor rnnDesc)
...
cpdef destroyPersistentRNNPlan(size_t rnnDesc):
    status = cudnnDestroyPersistentRNNPlan(<RNNDescriptor>rnnDesc)
    check_status(status)
